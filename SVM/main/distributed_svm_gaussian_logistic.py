...
from SVM.main.agent import Agent_subgrad, Agent_harnessing, Agent_harnessing_EventTriggered_trigger,Agent_harnessing_logistic
...
for p in range(m-1):
    ...
    ftmp += cvx.logistic(y1[p] *xc.T*Kc[p])
fopt = Ccvx * ftmp / M + cvx.quad_form(xc, Kc)/2
...
for i in range(n):
    ...
    s=0.01
    Agents.append(Agent_harnessing_logistic(n, m, agent_x[i], agent_y[i], s, Weight_matrix[i], i, bK[i], tK[i],C[i]))
...
for k in range(iteration):
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            for p in range(M):
                ...
                tmp = 1+np.exp(agent_y[j][p] *np.dot(bK[j][p],x_update))
                ftmp += np.log(tmp)
            f = f + C[j] * ftmp / M
            f+= np.dot(np.dot(tK[j], x_update), x_update) / 2
    ...
    print(normalized_f_er,f_er_base,f_er)
    # f_list.append(f)
