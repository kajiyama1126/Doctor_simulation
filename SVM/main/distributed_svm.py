...
from SVM.main.agent import Agent_subgrad, Agent_harnessing, Agent_harnessing_EventTriggered_trigger,Agent_subgrad_hinge
...
from SVM.main.Gaussian_kernel import GuassianKernelSVC,KernelSVC
...
for i in range(n):
    ...
    # # print(np.size(y1[p]))
    # # print(np.size(Kc[p]))
        ftmp_i[i] +=  cvx.pos(1 - agent_y[i][p] *agent_x[i][p]*xc)
    ftmp_i[i]+= 1/2*lam*cvx.norm(xc)
...
prob.solve(solver=cvx.ECOS,verbose=True, abstol=1e-20, reltol=1e-20, feastol=1e-20, max_iters=2500)
x_opt = xc.value
# for i in range(m):
#     x_opt[i] = xc.value
fopt = prob.value
...
for i in range(n):
    ...
    Agents.append(Agent_subgrad_hinge(n, m, agent_x[i], agent_y[i], s, Weight_matrix[i], i, lam))
...
for k in range(iteration):
    ...
    for i in range(n):
        ...
        for j in range(n):
            for p in range(M):
                ftmp += max(1 - agent_y[i][p] * np.dot(agent_x[i][p],x_update),0)

            ftmp += 1/2 *lam * np.linalg.norm(x_update,2)**2
