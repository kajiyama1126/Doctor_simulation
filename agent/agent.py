...
class Agent_harnessing(Agent):
    ...
    def grad(self):
        ...
        tmp = np.dot(self.A, self.x_i)
        # tmp = tmp.reshape([-1,1])
        grad = np.dot(A_to, (tmp - self.b))
...
class Agent_harnessing_diffusion(Agent_harnessing):
    def __init__(self, n, m, A, b, eta, weight, name,lam):
        self.lam = lam
        super(Agent_harnessing_diffusion, self).__init__(n, m, A, b,eta, weight, name)


    def initial_state(self):
        self.x_i = np.random.rand(1,self.m)
        self.x = np.zeros([self.n, 1,self.m])
        self.v_i = self.grad()
        self.v = np.zeros([self.n,1, self.m])
    ...
    def grad(self):
        ...
        tmp = np.dot(self.A, self.x_i.reshape([-1,1]))
        # tmp = tmp.reshape([-1,1])
        grad = np.dot(A_to, (tmp - self.b))+self.lam*self.x_i.reshape([-1,1])
...
class Agent_harnessing_diffusion_quantized(Agent_harnessing_diffusion):
    ...
    def make_h(self,k):
        self.h_x = self.h_x*0.999
        self.h_v = self.h_v*0.999
