# -*- coding: utf-8 -*-
from agent_senkou import Agent
from storage import Make_graph
import numpy as np
...
#エージェント定義
    agent0 = Agent([random.uniform(-5,5),random.uniform(-5,5)],n,0,graph0[0])
    agent1 = Agent([random.uniform(-5,5),random.uniform(-5,5)],n,1,graph0[1])
    agent2 = Agent([random.uniform(-5,5),random.uniform(-5,5)],n,2,graph0[2])
    agent3 = Agent([random.uniform(-5,5),random.uniform(-5,5)],n,3,graph0[3])
    agent4 = Agent([random.uniform(-5,5),random.uniform(-5,5)],n,4,graph0[4])
    ...
    for i in range(n):
        print all_agent[i].estimate_hat
...
print '平均収束値', value_sum/(test+1.)
print '平均反復回数' ,iteration_sum/(test+1.)
print '平均トリガ回数',trigger_sum/(test+1.)
# for k in range(n):
