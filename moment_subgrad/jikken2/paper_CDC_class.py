...
from moment_subgrad.jikken2.agent import new_Agent, new_Agent_moment_CDC2017_paper, new_Agent_moment_CDC2017_paper2
from moment_subgrad.jikken2.iteration import new_iteration_L1
...
class new_iteration_L1_paper3(new_iteration_L1):
    def make_agent(self, pattern):  # L1専用
        ...
        for i in range(self.n):
            if pattern % 2 == 0:
                Agents.append(
                    new_Agent(self.n, self.m, self.A[i], self.p[i], self.step[0], self.lamb, name=i, weight=None, R=self.R))
            elif pattern % 2 == 1:
                Agents.append(
                    new_Agent_moment_CDC2017_paper2(self.n, self.m, self.A[i], self.p[i], self.step[1], self.lamb, name=i,
                                                    weight=None, R=self.R))
    ...
    def make_graph(self, f_error):
        ...
        for i in range(int(self.pattern)):
            ...
            if i % 2 == 0:
                ...
                plt.plot(f_error[i], label=label[0] + stepsize, linestyle=line[0], linewidth=1)
                #plt.plot(f_error[i], label=label[0], linestyle=line[0], linewidth=1)
            if i % 2 == 1:
                stepsize = ' B = ' + str(self.B[i])
                plt.plot(f_error[i], label=label[1] + stepsize, linestyle=line[1], linewidth=1)
